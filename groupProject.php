...
$project_query = mysqli_query($conn, "SELECT * FROM projects WHERE project_id = $project_id");
...
<head>
    ...
    <style>
        ...
        .edit-project-form .file-input-container button {
            background-color: var(--dark);
            border: 1px solid #ccc; 
            color: #fff;
            width: 100%; 
...
<body>
    ...
    <div class="container">
        <div class="sidebar">
            ...
            <div class="form-container">
                <form method="POST" enctype="multipart/form-data" action="addTask.php?project_id=<?php echo $project_id; ?>"> 
        ...
        <div class="info">
            <div class="project-info">
                ...
                <div class="icons">
                    ...
                    <a href="#" class="icon list-members-icon" id="list-members-button"><i class="fas fa-users"></i></a>
                    <div id="member-list-modal" class="modal">
                        <div class="modal-content">
                            ...
                            <ul>
                                <?php
                                    $members_query = mysqli_query($conn, "SELECT * FROM project_members WHERE project_id = $project_id");
                                    $members = mysqli_fetch_all($members_query, MYSQLI_ASSOC);
                                    foreach ($members as $member) {
                                        $user_id = $member['user_id'];
                                        $user_query = mysqli_query($conn, "SELECT * FROM users WHERE unique_id = $user_id");
                                        $user_details = mysqli_fetch_assoc($user_query);
                                        if ($user_details) {
                                            $username = htmlspecialchars($user_details['fname']);
                                            $lastname = htmlspecialchars($user_details['lname']);
                                            $email = htmlspecialchars($user_details['email']);
                                            ?>
                                            <li>
                                                <?php echo "$username $lastname ($email)"; ?>
                                                <a href="viewProfile.php?creator_id=<?php echo $user_id; ?>"><i class="fas fa-eye"></i></a>
                                            </li>
                                            <?php
                                        }
                                    }
    ...
    <div class="edit-project-form" style="display:none;">
        ...
        <form method="POST" action="ediProject.php" enctype="multipart/form-data">
            <input type="hidden" name="project_id" value="<?php echo $project_id; ?>">
...
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const listMembersButton = document.getElementById('list-members-button');
            const memberListModal = document.getElementById('member-list-modal');
            const closeBtn = document.querySelector('.close');

            listMembersButton.addEventListener('click', function() {
